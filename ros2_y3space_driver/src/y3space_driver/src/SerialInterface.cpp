...
SerialInterface::~SerialInterface()
{
    if (m_connection != NULL)
    {
      	if(m_connection->isOpen())
      	{
			...
			// RCLCPP_INFO(this->m_logger <"Closing the Serial Port");
			// RCLCPP_INFO(this->get_logger, "%s Closing the Serial Port", m_logger.c_str());
			 RCLCPP_INFO(m_logger, "Closing the Serial Port");
        	m_connection->close();
      	}
    }
}
...
void SerialInterface::serialConnect()
{
	...
	catch(IOException &e)
	{
		...
		// RCLCPP_ERROR(this->m_logger, "Unable to connect port: %s", m_port.c_str());
		// RCLCPP_ERROR(this->m_logger, "Is the device pluggged in? Is the serial port open?\nError: %s", ioerror.c_str());
	}
	...
	if(m_connection && m_connection->isOpen())
	{
		...
		// RCLCPP_INFO(this->m_logger, "Connection Established with Port: %s  with baudrate: %d", m_port.c_str(), m_baudrate);
	}
}
...
void SerialInterface::serialWrite(uint8_t *buf, size_t len)
{
    ...
    if (written != len)
    {
		...
		// RCLCPP_WARN(this->m_logger, "Len: %d .. Written: %d", len, written);
    }
}
...
uint8_t SerialInterface::serialReadByte()
{
    ...
    if(this->m_connection->available() > 0)
    {
		...
		if (bytes != 1)
		{
			// RCLCPP_WARN(this->m_logger, "Unable to read");
		}
    }
    return buf;
}
